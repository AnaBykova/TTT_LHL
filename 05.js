...
const stdev = function(arr) {
  ...
  const mean = arr.reduce((sum, number) => sum + number, 0) / arr.length;

  // Step 2: Calculate the squared deviations and sum them up
  const sumOfSquaredDeviations = arr.reduce((sum, number) => sum + Math.pow(number - mean, 2), 0);

  // Step 3: Divide the sum of squared deviations by the number of values
  const stdevValue = Math.sqrt(sumOfSquaredDeviations / arr.length);

  // Step 4: Round the final value before returning
